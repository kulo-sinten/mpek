...
surat=$(curl -sS https://raw.githubusercontent.com/kulo-sinten/mpek/main/REG/authbot  | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -qO- https://raw.githubusercontent.com/kulo-sinten/mpek/main/bot_panel/ShellBot.sh >/etc/.maAsiss/.Shellbtsss
			}
